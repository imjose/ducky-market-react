...
export default function Page({
  ...
  function onPostTransaction(selectedProducts: { [key: string]: number }, totalAmount: number) {
    postTransaction(selectedProducts, totalAmount).then(generatedTransaction => {
      ...
      if (generatedTransaction.value > (mostExtpensiveTransactionRef.current?.value ?? 0)) {
  ...
  return (
    <div className="w-full flex flex-row flex-wrap">
      <div className={clsx("w-full p-2", { "md:w-2/3": lastTransactionNode || mostExpensiveTransactionNode })}>
        <Products products={products} productMap={productMap} submitTransaction={onPostTransaction} />
      </div>
